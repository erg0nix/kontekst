...
func setupBashCommand(t *testing.T, dir, name, script string, manifest string) *command.Registry {
	t.Helper()
...
func setupPythonCommand(t *testing.T, dir, name, script string, manifest string) *command.Registry {
	t.Helper()
	cmdDir := filepath.Join(dir, name)
	writeTestFile(t, filepath.Join(cmdDir, "command.toml"), manifest)
...
`)
	...
	tool := &CommandTool{Registry: reg}
	ctx := toolpkg.WithWorkingDir(context.Background(), agentDir)
