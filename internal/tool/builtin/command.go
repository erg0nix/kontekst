...
func (tool *CommandTool) Preview(args map[string]any, ctx context.Context) (string, error) {
	...
	if cmd.WorkingDir == "agent" {
		if agentDir := toolpkg.WorkingDir(ctx); agentDir != "" {
			workDir = agentDir
		}
	}
...
func resolveArguments(cmd *command.Command, args map[string]any) map[string]string {
...
func validateArguments(cmd *command.Command, args map[string]any) (map[string]string, error) {
...
func buildEnv(cmd *command.Command, resolvedArgs map[string]string, ctx context.Context) []string {
	var env []string
...
func interpreterAndScript(cmd *command.Command) (string, string) {
	switch cmd.Runtime {
	case "python":
...
func resolveWorkDir(cmd *command.Command, ctx context.Context) string {
	if cmd.WorkingDir == "agent" {
		if agentDir := toolpkg.WorkingDir(ctx); agentDir != "" {
			return agentDir
		}
...
func ensureVenv(ctx context.Context, cmd *command.Command) error {
...
func RegisterCommand(registry *toolpkg.Registry, commandsRegistry *command.Registry) {
